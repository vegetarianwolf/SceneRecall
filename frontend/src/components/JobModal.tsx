...
import { DEFAULTS, type Asset, type Estimate, type JobPayload } from '../types';
...
export default function JobModal({ asset, onClose }: { asset: Asset; onClose: () => void }) {
  ...
  const payload = useMemo<JobPayload>(() => ({ asset_id: asset.id, stages: [...(vision ? ['vision' as const] : []), ...(subtitle ? ['subtitle' as const] : [])], start_ms: Math.round(start * 1000), end_ms: end ? Math.round(Number(end) * 1000) : null, window_ms: Math.round(windowSeconds * 1000), frames_per_window: frameCount, subtitle_fps: fps, subtitle_crop: crop, max_requests: requestLimit, max_cost: costLimit ? Number(costLimit) : null, force }), [asset.id, vision, subtitle, start, end, windowSeconds, frameCount, fps, crop, requestLimit, costLimit, force]);
